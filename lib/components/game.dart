...
class MyPhysicsGame extends Forge2DGame {
  ...
  FutureOr<void> onLoad() async {
    ...
    aliens = XmlSpriteSheet(aliensImage,
        await rootBundle.loadString('assets/images/spritesheet_aliens.xml'));
    elements = XmlSpriteSheet(elementsImage,
        await rootBundle.loadString('assets/images/spritesheet_elements.xml'));
    tiles = XmlSpriteSheet(tilesImage,
        await rootBundle.loadString('assets/images/spritesheet_tiles.xml'));
  ...
  Future<void> addBricks() async {
    for (var i = 0; i < 5; i++) {
      ...
      final damage = BrickDamage.randomDamage;
      await world.add(
        Brick(
          type: type,
          size: size,
...
Future<void> addPlayer() async {
  final colour = PlayerColor.randomColor;
  return world.add(          
        Player(
          Vector2(camera.visibleWorldRect.left * 2 / 3, 0),
          aliens.getSprite(colour.fileName),
          colour,
        ),
      );
}
   ...
   if (isMounted &&
      ...
      world.addAll(
        [
          ...
          (position: Vector2.zero(), color: const ui.Color.fromARGB(255, 255, 64, 64)),
        ].map(
          (e) => TextComponent(
            text: 'You win!',
